...
(t/deftest rendering
  (t/testing "html helper fns"
    ...
    (t/is (= [:table [:thead [:tr [:th "name"] [:th "desc"] [:th "id"]]]
              [:tbody
               [:tr [:th {:scope "row"} "Entry 1"] [:td "First item"] [:td 245]]
               [:tr [:th {:scope "row"} "Entry 2"] [:td "Second item"]
                [:td 249]]]]
             (map->table {"Entry 1" {:desc "First item" :id 245}
                          "Entry 2" {:desc "Second item" :id 249}})))
    (t/is
     (= [:table [:thead [:tr [:th "name"] [:th "desc"] [:th "id"]]]
         [:tbody [:tr [:th {:colspan 3} "I"]]
          [:tr [:th {:scope "row"} "Entry 1"] [:td "First item"] [:td 245]]
          [:tr [:th {:scope "row"} "Entry 2"] [:td "Second item"] [:td 249]]]
         [:tbody [:tr [:th {:colspan 3} "J"]]
          [:tr [:th {:scope "row"} "Entry 3"] [:td "Third item"] [:td 252]]]]
    ...
    (t/is (= [:table
              ...
              [:tbody [:tr [:th {:colspan 4} "I"]]
               [:tr [:th {:scope "row"} "Entry 1"] [:td "First item"] [:td 245]
                [:td ""]]
               [:tr [:th {:scope "row"} "Entry 2"] [:td "Second item"] [:td 249]
                [:td ""]]]
              [:tbody [:tr [:th {:colspan 4} "J"]]
               [:tr [:th {:scope "row"} "Entry 3"] [:td "Third item"] [:td 252]
                [:td "abc"]]]]
             (map->table
              {"Entry 1" {:desc "First item" :id 245 :group "I"}
               "Entry 2" {:desc "Second item" :id 249 :group "I"}
               "Entry 3" {:desc "Third item" :id 252 :group "J" :meta "abc"}}
              :group)))
    (t/is (= (html [:r-cell {:span 3} "ab"]) "<r-cell span=\"3\">ab</r-cell>"))
  ...
  (t/testing "source code fns"
    #_(t/is (= [:pre
                [:code {:class "language-clojure"}
                 "(def delimiters [\"✳\" \"🔚\"])"]]
               (include-def {:render-fn str}
                            'delimiters
                            "./src/respatialized/parse.clj")))
    #_(t/is
       (=
        [:pre
         [:code {:class "language-clojure"}
    ...
    #_(t/is
       (= [:pre
          ...
          (include-def {:render-fn str}
                       'hiccup-form?
                       "./src/respatialized/document.clj")))))
