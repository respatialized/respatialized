(ns respatialized.build-test
  #_(:require [respatialized.build :refer :all]
...
;;               [:schema [:cat [:enum :pre] [:schema [:cat [:enum :code]
;;               :string]]]]]]]])
...
;;         (let [{:keys [site.fabricate.page/title
;;         site.fabricate.page/evaluated-content]
;;                :or {title p}
...
;;                 (str "Post " title " had " (count errors) " evaluation
;;                 errors")))))))
