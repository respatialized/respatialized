...
(def remedy
  ...
  [#_[:* (before) (after) {:box-sizing "border-box"}] ; unclear if I got the
   ...
   [:picture {:display "contents"}] [:source {:display "none"}]
   [:img :svg :video :canvas {:height "auto"}] [:audio {:width "100%"}]
   [:img {:border-style "none"}] [:svg {:overflow "hidden"}]
...
(def page-style
  (list
    ...
    [:header
     {:font-weight "600", :grid-column "1 / -1", :font-family "Def Sans"}]
    [:article
    ...
    [:dl {:grid-column-start 1, :display "contents"}]
    [:dt {:margin-bottom "0.2rem", :hyphens "none"}]
    [:dd {:margin-bottom "0.4rem"}]
    [:.fabricate-error {:display "contents"}
     [:h6
      {:margin-top "0.1em",
       :margin-bottom "0.1em",
     ...
     [:dl {:display "contents"} [:dt {:grid-column "1 / 1"}]
      [:dd {:grid-column "2 / -1"}]] [:pre {:grid-column "1 / -1"}]
     [:details {:display "contents"} [:summary {:grid-column "1 / -1"}]]]
    ...
    [:figure {:display "contents"} [:a {:display "contents"}]]
    [:table
     {:font-size "0.95rem",
      :letter-spacing "0.01rem",
      :line-height "1.45em",
      :max-width "90em"}]
    [:article [:ol :ul {:display "contents"} [:li {:grid-column "1 / -1"}]]]
    [:td :th
     {#_#_:border "0.5px solid", :padding "0.25rem", :text-align "left"}]
    [(select/attr "colspan")
     {:text-align "center",
