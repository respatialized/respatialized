(ns respatialized.archive
  ...
  (:require [respatialized.document :as doc]
...
;; if I think about it more, then all that's needed to uniquely identify a
;; revision is:
;; - git sha
;; - file hash
;; - page id (page id can be derived from filename recorded in DB in a stateful
;; context)

;; Asami does support these types for :db/ident and :id:
;; https://github.com/threatgrid/asami/issues/168
;; so it may be better to derive the uniqueness of revisions from observable
;; facts.
;; the :id attribute is observable, so it is more appropriate to use here than
;; :db/ident
;; (which isn't). this is probably erring on the side of redundancy, but that's
;; OK.

(def sha-schema
  (m/schema [:string {:min 40, :max 40, :description "A 40-character SHA"}]))

(def revision-entity-schema
  (let [file-sha
          (mu/update-properties sha-schema assoc :description "A git file SHA")
        repo-sha (mu/update-properties sha-schema
                                       assoc
                                       :description
                                       "A git commit SHA")]
    (m/schema
      [:map
       [:id [:tuple [:uuid {:description "The page ID"}] file-sha repo-sha]]
       [:file/path [:string {:description "The path of the input file"}]]
       [:page/id [:uuid {:description "The page ID"}]] [:git/sha repo-sha]
       [:git/file-hash file-sha]
       [:git/worktree-status
        [:keyword
         {:description
            "A description of the worktree status at the time a post was recorded"}]]
       [::revision-time
        [:fn
         {:description "The time a given page's revision entered the database"}
         inst?]]
       [::revision-index
        [:int
         {:description
            "A sequentially increasing index number for page revisions"}]]
       [::revision-new?
        [:boolean
         {:description
            "A boolean value indicating whether the revision has changed"}]]])))
...
(defn git-sha
  ...
  ([] (str/trim-newline (:out (sh/sh "git" "log" "--format=%H" "-n" "1"))))
  ([filename]
   (str/trim-newline
    (:out (sh/sh "git" "log" "--format=%H" "-n" "1" "--" filename)))))

(defn file-hash
  [filename]
  (str/trim-newline (:out (sh/sh "git" "hash-object" filename))))
...
(defn file->revision
  ...
  ([file db]
   (let [now (Instant/now)
         ...
         [[page-id r-ix rev-hash]]
         #_existing-post?
         (d/q '[:find ?id ?r-ix ?hash :in $ ?path :where [?p :file/path ?path]
                [?p :page/id ?id] [?p ::revision-index ?r-ix]
...
(comment
  ...
  (def example-page
    (get @site.fabricate.prototype.write/pages
...
(defn html-attr->kw
  [attr-name]
  (cond (keyword? attr-name) (keyword "html.attribute" (name attr-name))
        (.startsWith attr-name "data-")
          (keyword "html.attribute.data" (second (str/split attr-name #"-")))
        :else (keyword "html.attribute" attr-name)))

(defn attrs->ns-map
  [attrs]
  (into {} (map (fn [[a v]] [(html-attr->kw a) v]) attrs)))

(defn parsed->asami
  [{:keys [tag attrs contents]}]
  (merge {:html/contents contents} {:html/tag tag} (attrs->ns-map attrs)))

(def parsed-html-schema
  (m/schema
    [:schema
     {:registry {"element" [:map [:tag :keyword] [:attrs [:or :map :nil]]
                            [:contents
                             [:*
                              [:or html/atomic-element
                               [:schema [:ref "element"]]]]]]}} "element"]))
...
(defn page->asami
  [{:keys [site.fabricate.page/evaluated-content
           site.fabricate.file/input-filename site.fabricate.page/title
           site.fabricate.page/namespace site.fabricate.page/metadata],
    :as page-map}]
  (let [parsed (page-parser evaluated-content)]
    (when (not= ::m/invalid parsed)
      (->> parsed
           (clojure.walk/postwalk
             #(if (and (map? %) (every? #{:tag :attrs :contents} (keys %)))
                (parsed->asami %)
                %))
           (merge (select-keys page-map [:site.fabricate.page/title]))))))

(defn replacement-annotation
  [kw]
  (->> kw
       str
       (drop 1)
...
(defn record-page!
  ...
  [{:keys [site.fabricate.page/evaluated-content
           ...
           site.fabricate.page/unparsed-content site.fabricate.file/input-file
           site.fabricate.page/title],
    :as page-data} conn]
  (let [revision-entity (file->revision input-file conn)
        ;; [BUG] - replacement annotations can't be used on non-existent
        ;; entities so there needs to be a conditional workaround for now
        page-ent-data (if (d/entity conn (:page/id revision-entity))
                        {:id (:page/id revision-entity),
                         :page/revisions+ {:id (:id revision-entity)}, ; append
                                        ; revision
...
(comment
  ;; placeholder for functions that should eventually exist but don't work
  ;; yet
  (defn hiccup->asami
    []
    (mt/transformer {:decoders {:html/atomic-element {:compile (fn [schema _]
                                                                 identity)}}}))
  (defn asami->hiccup [n] nil))

(defn query->table
  [q db {:keys [col-renames], :or {col-renames {}}, :as opts}]
  (let [res (d/q q db)
        {:keys [cols]} (meta res)
        header (into [:thead]
                     (mapv (fn [col-name] [:th
                                           (str (col-renames col-name
                                                             col-name))])
                       cols))
        rows (mapv (fn [r] (reduce conj [:tr] (mapv (fn [rv] [:td rv]) r)))
               res)]
    (reduce conj [:table header] rows)))

(comment
  (d/q '[:find ?e ?a :where [?e ?a true]] test-db)
  (d/q '[:find ?e ?a :where [?e ?a true]] test-db)
  @(d/transact test-db [{"some-attr" "some-value"}])
  (query->table '[:find ?title ?fp :where [?e :file/path ?fp]
                  [?e :respatialized.writing/title ?title]]
                (d/db db)
                {:col-renames '{?title "Title", ?fp "file path"}})
  (d/q '[:find ?a #_?v :where
  ...
  (do @(d/import-data test-db (d/export-data db)) nil)
  (d/delete-database "asami:mem://test-db")
  (d/q '[:find ?a ?v :where
         [?e :site.fabricate.page/title "This Website Is Not A Tree"]
         [?e ?a ?v]]
       (d/db test-db))
  (keys (get @site.fabricate.prototype.write/state :site.fabricate/pages))
  (get-in @site.fabricate.prototype.write/state
          [:site.fabricate/pages "./content/relay.html.fab"])
  (def posts
  ...
  (let [articles (->> (get @write/state :site.fabricate/pages)
                      vals
                      (map (fn [{:keys [site.fabricate.page/evaluated-content
                                        site.fabricate.page/title
                                        site.fabricate.file/filename]}]
                             (when evaluated-content
                               (let [asami-content (contents->asami
                                                    evaluated-content)]
                                 (when (not= :malli.core/invalid asami-content)
                                   (assoc asami-content
                                          ...
                                          :filename filename))))))
                      (filter some?))]
    (d/transact-async test-db {:tx-data articles, #_#_:executor exec}))
  (d/q '[:find ?c :tg/contains ?q :where [?e :html/tag :blockquote]
         [?e :html/tag ?t] [?e :html/contents+ ?c] [?c :tg/contains+ ?q]]
       (d/db test-db))
  (d/q '[:find ?title ?v :where [?p :respatialized.writing/title ?title]
         [?p ?a* ?e] (or [?e :html/tag :blockquote] [?e :html/tag :q])
         [?e ?a2* ?v] [(string? ?v)]]
       (d/db test-db))
