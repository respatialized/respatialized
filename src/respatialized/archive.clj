...
(def revision-entity-schema
  (let [file-sha (mu/update-properties sha-schema
                                       assoc
                                       :description
                                       "A git file SHA")
        repo-sha (mu/update-properties sha-schema
                                       ...
                                       "A git commit SHA")]
    (m/schema
     [:map [:id [:tuple [:uuid {:description "The page ID"}] file-sha repo-sha]]
...
(def parsed-html-schema
  (m/schema [:schema
             {:registry
              {"element"
               [:map [:tag :keyword] [:attrs [:or :map :nil]]
                [:contents
                 [:* [:or html/atomic-element [:schema [:ref "element"]]]]]]}}
             "element"]))
...
(defn record-page!
  ...
  (let [revision-entity (file->revision input-file conn)
    ...
    (if (::revision-new? revision-entity)
      #_(u/trace ::record-page-revision!
                 {:pairs [:site.fabricate.page/title title]})
      (let [{:keys [tx-data] :as r} @(d/transact conn
                                                 [revision-data page-ent-data])]
  ...
  (let [res            (d/q q db)
        ...
        header         (into [:thead]
                             (mapv (fn [col-name] [:th
                                   ...
                                   cols))
        rows           (mapv (fn [r]
                               (reduce conj [:tr] (mapv (fn [rv] [:td rv]) r)))
                             res)]
