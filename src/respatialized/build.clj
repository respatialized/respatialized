...
;; (defn new-page?
;;   [{:keys [site.fabricate.file/input-file] :as page-data}
;;    {:keys [site.fabricate.app/database] :as application-state-map}]
;;   (let [{:keys [::archive/revision-new?] :as rev-entity}
;;         (archive/file->revision input-file (:db/conn database))]
;;     revision-new?))
...
;;                     (slurp input-file))
;;              (do
;;                (println "Page at" (.toString input-file) "up to date,
;;                skipping")
;;                page-data)))
...
;;                :as   page-data}
;;               {:keys [site.fabricate.app/database] :as
;;               application-state-map}]
;;            (do (println "writing page content to" output-file)
...
;;                 :site.fabricate.server/config        {:cors-allow-headers
;;                 nil
;;                                                       :dir (str
;;                                                             (System/getProperty
;;                                                              "user.dir")
...
;;                                                       :port 8000
;;                                                       :no-cache true}}
;;                :site.fabricate.app/database {:db/uri  archive/db-uri
;;                                              :db/conn nil}}))
...
;;                     @write/state)
;;       (println "done"))
;;   (do (fsm/complete operations "content/not-a-tree.html.fab" initial-state)
;;       (println "done"))
;;   (do (fsm/complete operations "content/holotype4.html.fab" initial-state)
;;       (println "done")))
...
;;              %
;;              (fn [ag ^Throwable ex] (.printStackTrace ex) ag))
;;             %))
;;       (send-off (fn [{:keys [site.fabricate.app/database] :as s}]
;;                   (assoc-in s
;;                    [:site.fabricate.app/database :db/conn]
...
(comment
  ...
  (-> write/state
      ...
      (send-off write/draft!)
      #_(send-off
         (fn [{:keys [site.fabricate/settings] :as application-state-map}]
           application-state-map
           ...
           #_(let [output-dir       (:site.fabricate.file/output-dir settings)
               ...
               (assoc application-state-map
                      ...
                      (watch-dir (fn [{:keys [file count action]}]
