...
publish: ## Rsync the website to the public web server (does not rebuild site first)
	...
	raco pollen publish . ~/publish; \
    rsync -av ~/publish/ $(BLOG_SRV) --delete --exclude=projects --exclude=.git --exclude=drafts --exclude=pollen-local --exclude='*.rebuild' --exclude=.DS_Store --exclude=.gitignore --exclude='template*.*' --exclude=makefile --exclude=util --exclude='posts/img/originals'; \
    rm -rf ~/publish
